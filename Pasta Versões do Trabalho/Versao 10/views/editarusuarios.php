...
<body>
    ...
    <?php if ($action === 'list'): ?>
        <a href="inserirusuarios.html?tabela=<?= $tabela ?>">Adicionar <?= $tabela ?></a>
        <table>
            <thead>
                <tr>
                    <th>ID</th>
            ...
            <tbody>
                <?php foreach ($registros as $registro): ?>
                    <tr>
                        ...
                        <?php endforeach; ?>
                        <td>
                            <a href="editarusuarios.php?action=edit&tabela=<?= $tabela ?>&id=<?= $registro['ID_Usuario'] ?>">Editar</a>
                            <a href="editarusuarios.php?action=delete&tabela=<?= $tabela ?>&id=<?= $registro['ID_Usuario'] ?>" onclick="return confirm('Tem certeza que deseja excluir este usuário?')">Excluir</a>
                        </td>
                    </tr>
                <?php endforeach; ?>
            </tbody>
    ...
    <?php elseif ($action === 'add' || $action === 'edit'): ?>
        <form action="editarusuarios.php?action=<?= $action ?>&tabela=<?= $tabela ?>&id=<?= $id ?>" method="post">
            <?php foreach ($campos as $campo): ?>
                <label for="<?= $campo ?>"><?= htmlspecialchars($campo) ?>:</label>
                <input type="text" id="<?= $campo ?>" name="<?= $campo ?>" value="<?= htmlspecialchars($registro[$campo] ?? '') ?>" required>
            <?php endforeach; ?>
